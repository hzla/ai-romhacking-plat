...
| "Only allow certain natures" | `wildNatures` | Requires nature ID list in the recipe. |
...
| "Add every Gen 5 Pokemon" | expansion project | Too broad for one recipe; suggest a smaller slice such as researching species limits or adding one test species. |
...
"The toolkit does not have a recipe for that yet, but this looks like a localized code change. I will research the relevant decomp files, make it a reusable capability, and then add a recipe once it is tested."
